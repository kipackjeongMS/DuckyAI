---
name: duckyai-cli
description: DUCKYAI CLI for orchestrator configuration, worker management, vault updates, and CLI workflow integration.
metadata:
  version: 3.0.0
  author: lifidea
  created: 2025-12-10
...
This skill provides guidance for configuring and managing DUCKYAI orchestrator, workers, and vault updates. It helps users set up agents, configure multi-worker execution, manage CLI workflows, and update the vault from GitHub releases.

## DUCKYAI Architecture
...
"설정 보기" → Show Current Config (duckyai --show-config)
"에이전트 목록" → List Agents (duckyai --list-agents)
...
| `output_path` | Worker별 출력 경로 | `03-Knowledge/Documentation/Claude` |
...
- type: agent
  ...
  workers:
    - executor: gemini_cli
      label: Gemini
      output_path: 03-Knowledge/Documentation/Gemini
    - executor: claude_code
      label: Claude
      output_path: 03-Knowledge/Documentation/Claude
    - executor: codex_cli
      label: Codex
      output_path: 03-Knowledge/Documentation/Codex
```
...
| Command | 설명 |
|---------|------|
| `duckyai --show-config` | 현재 설정 보기 |
| `duckyai --list-agents` | 에이전트 목록 |
| `duckyai --run <agent>` | 특정 에이전트 실행 |
| `duckyai --enable <agent>` | 에이전트 활성화 |
| `duckyai --disable <agent>` | 에이전트 비활성화 |
| `duckyai --status` | 실행 상태 확인 |
| `duckyai --logs` | 로그 확인 |
...
**Purpose**: Add, update, and track tasks in DUCKYAI system
...
Agent: 어떤 에이전트를 추가할까요?
       - 파일 기반 (input_path 필요)
       - 스케줄 기반 (cron 필요)
User: 파일 기반, 00-Inbox 폴더 감시해서 03-Knowledge/Documentation에 요약 저장
Agent: 다음 설정으로 추가할게요:
...
Agent: 각 모델 결과를 어디에 저장할까요?
       예: 03-Knowledge/Documentation/Claude, 03-Knowledge/Documentation/Gemini
```
...
### Design Documents
- `[[03-Knowledge/Documentation/2025-12-10 DUCKYAI Onboarding Agent Design - Claude Code]]`
...
**"에이전트가 실행 안 돼요"**
→ `duckyai --status`로 상태 확인 후, 로그 확인: `duckyai --logs`
...
**"cron이 트리거 안 돼요"**
→ cron 표현식 문법 확인, `duckyai --status`로 다음 실행 시간 확인

**"설정 변경이 반영 안 돼요"**
→ orchestrator 재시작 필요: `duckyai --restart`
