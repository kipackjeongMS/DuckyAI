  ...
  const api: DuckyAIApi = {
    ...
    chat: {
      send: async (text: string) => {
        // Route through daemon's chat endpoint if available
        try {
          const resp: any = await post("/api/chat/send", { message: text });
          return resp.response ?? JSON.stringify(resp);
        } catch {
          emitNotification({
            type: "error",
            title: "Chat unavailable",
            body: "The chat engine is not running. Start the orchestrator first.",
          });
          return "Chat engine not available. Please ensure the DuckyAI daemon is running.";
        }
      },
    },
