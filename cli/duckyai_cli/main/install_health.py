...
PACKAGE_IMPORT_NAME = "duckyai_cli"
REPO_PTH_FILENAME = "duckyai_cli_repo.pth"
...
def is_source_checkout(path: Path) -> bool:
    ...
    return path.is_dir() and (path / "pyproject.toml").exists() and (path / "duckyai_cli").is_dir()
...
def can_import_package_outside_checkout() -> tuple[bool, str]:
    """Verify that a fresh Python process can import duckyai_cli outside the repo."""
    probe = subprocess.run(
        [sys.executable, "-c", "import duckyai_cli"],
        capture_output=True,
        text=True,
        cwd=str(Path.home()),
        timeout=10,
...
def get_duckyai_launch_cmd(*args: str) -> list[str]:
    ...
    return [sys.executable, "-m", "duckyai_cli", *args]
...
def repair_source_install(source_dir: Path | None = None) -> Path:
    ...
    if not import_ok:
        raise RuntimeError(error_text or "duckyai_cli is still not importable after repair")
    return pth_path
