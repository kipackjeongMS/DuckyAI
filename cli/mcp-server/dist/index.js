...
const PLAYBOOK_TEMPLATES_DIR = path.resolve(path.dirname(new URL(import.meta.url).pathname.replace(/^\/([A-Z]:)/, '$1')), '..', '..', 'duckyai_cli', '.playbook', 'templates');
...
async function buildDailyNoteFromTemplate(targetDate, carrySection) {
...
async function extractCarryForward(filePath) {
    try {
...
async function ensureContactExists(personName, context) {
...
// =============================================================================
server.tool("getCurrentDate", "Get the current date and time in the user's configured timezone. Use this to verify date calculations.", {}, async () => {
    const tz = await getUserTimezone();
    const now = new Date();
...
// =============================================================================
server.tool("convertUtcToLocalDate", "Convert a UTC timestamp to the user's local date (YYYY-MM-DD) and time (HH:MM). Use this for EVERY timestamp from WorkIQ or Teams data before grouping by date.", {
...
// =============================================================================
server.tool("prepareDailyNote", "Create today's daily note with carry-forward items from yesterday", {
    date: z
        .string()
        .optional()
...
// SKILL: logPRReview
// =============================================================================
server.tool("logPRReview", "Log a PR review or comment to today's daily note", {
...
`;
        ...
        if (!contentLower.includes(`pr ${prNumber.toLowerCase()}`)) {
            ...
            if (prSectionMatch) {
                ...
                const newContent = existing === "- [ ]" || existing === "-" || existing === ""
                    ? pendingEntry
                    : `${existing}\n${pendingEntry}`;
                content = content.replace(prSectionMatch[0], `${prSectionMatch[1]}${newContent}\n\n${prSectionMatch[3]}`);
            }
        }
    }
    else {
    ...
    const createdContact = await ensureContactExists(person, `First referenced in [PR ${prNumber}](${prUrl}) - ${description}`);
...
// SKILL: logAction
// =============================================================================
server.tool("logAction", "Log a completed action to today's daily note", {
    ...
    if (addToCarryForward) {
        ...
        content = content.replace(/(### Carry forward to tomorrow\n)([\s\S]*?)$/, (match, header, existing) => {
...
// SKILL: createTask
// =============================================================================
server.tool("createTask", "Create a new task in 01-Work/Tasks/", {
...
// SKILL: logTask
// =============================================================================
server.tool("logTask", "Add a pending task to the ## Tasks section of today's daily note as a linked item", {
    ...
    if (tasksMatch) {
        ...
        const newContent = existing === "- [ ]" || existing === ""
            ? logEntry
            : `${existing}\n${logEntry}`;
        content = content.replace(tasksMatch[0], `${tasksMatch[1]}${newContent}\n\n${tasksMatch[3]}`);
        await fs.writeFile(dailyPath, content, "utf-8");
    }
    else {
        return {
...
// SKILL: archiveTask
// =============================================================================
server.tool("archiveTask", "Move a completed/cancelled task to 05-Archive/", {
...
// =============================================================================
server.tool("updateTaskStatus", "Update the status of a task", {
    title: z.string().describe("Task title (filename without .md)"),
...
// SKILL: createMeeting
// =============================================================================
server.tool("createMeeting", "Create a new meeting note in 02-People/Meetings/", {
...
// SKILL: create1on1
// =============================================================================
server.tool("create1on1", "Create a new 1:1 meeting note in 02-People/1-on-1s/", {
    ...
    // Update the title
    template = template.replace(/^# 1:1 - .*$/m, `# 1:1 with [[${person}]] - ${meetingDate}`);
...
// =============================================================================
server.tool("prepareWeeklyReview", "Create a weekly review note with aggregated data from daily notes", {
    ...
    for (const file of dailyFiles) {
        ...
        if (fileDate >= mondayStr && fileDate <= fridayStr) {
            const content = normalizeLineEndings(await fs.readFile(path.join(DAILY_DIR, file), "utf-8"));
            // Extract completed tasks
            const tasksMatch = content.match(/## Tasks Completed\n([\s\S]*?)(?=\n## )/);
            if (tasksMatch) {
                const tasks = tasksMatch[1].trim().split("\n").filter(line => line.startsWith("- [x]") && line !== "- [x]");
    ...
    if (completedTasks.length > 0) {
        ...
        template = template.replace(/^(## Key Accomplishments\n)- $/m, `$1${tasksSection}`);
    }
    await fs.writeFile(filePath, template, "utf-8");
    return {
        content: [
...
// SKILL: triageInbox
// =============================================================================
server.tool("triageInbox", "Scan 00-Inbox/ and categorize items into appropriate vault folders (Tasks, Knowledge, Meetings, etc.)", {
    dryRun: z.boolean()
        .default(true)
...
// SKILL: enrichNote
// =============================================================================
server.tool("enrichNote", "Enrich a note by adding frontmatter, wiki links, summary section, and structure improvements", {
...
// =============================================================================
server.tool("updateTopicIndex", "Update or create a topic index file in 03-Knowledge/Topics/ by scanning for related notes across the vault", {
    ...
    if (existingContent) {
        ...
        if (/^## Related Notes/m.test(existingContent)) {
            newContent = existingContent.replace(/^## Related Notes[\s\S]*?(?=\n## |\n---|\Z)/m, `## Related Notes\n\n${refsSection}\n`);
        }
        else {
...
// =============================================================================
server.tool("generateRoundup", "Generate a rich daily roundup by aggregating completed tasks, meetings, and key notes from today's daily note and vault activity", {
    date: z.string()
        .optional()
    ...
    for (const line of lines) {
        if (/^##\s/.test(line)) {
            ...
            if (heading.includes("completed") || heading.includes("done"))
                currentSection = "completed";
            else if (heading.includes("meeting"))
                currentSection = "meetings";
            else if (heading.includes("note") || heading.includes("log"))
                currentSection = "notes";
            else if (heading.includes("carry") || heading.includes("tomorrow"))
                currentSection = "carry";
            else
                currentSection = "";
            continue;
        }
...
// =============================================================================
server.tool("getTeamsChatSyncState", "Get the last Teams chat sync timestamp (watermark). Returns ISO timestamp of last successful sync.", {}, async () => {
...
server.tool("updateTeamsChatSyncState", "Update the Teams chat sync watermark after a successful sync", {
    ...
    let existing = { processedThreads: [], syncCount: 0 };
    try {
        const raw = await fs.readFile(stateFile, "utf-8");
...
// =============================================================================
server.tool("updateDailyNoteSection", "Update a specific H2 section in a daily note. Replaces the entire content of that section. Use this for precise updates (e.g. Teams Chat Highlights) where you want full control over merging.", {
...
function deduplicateHighlights(existing, incoming) {
    ...
    const sectionOrder = []; // To preserve order of existing sections
                ...
                for (const line of lines) {
                    if (line.match(/^- /) && !line.match(/^  /)) {
                        ...
                        if (currentBlock.length > 0) {
                            ...
                            if (blockText)
                                sections.get(key).blocks.push(blockText);
                        }
                        currentBlock = [line];
                    }
                    else if (line.trim()) {
...
// =============================================================================
server.tool("appendTeamsChatHighlights", "Append a Teams Chat Highlights section to today's daily note. (Legacy wrapper around updateDailyNoteSection)", {
        ...
        for (const { name, note } of personNotes) {
            ...
            try {
                ...
                if (/^## Notes/m.test(contactContent)) {
                    contactContent = contactContent.replace(/^(## Notes\n)([\s\S]*?)$/m, (match, header, existing) => {
                        const trimmed = existing.trimEnd();
                        // Avoid duplicate entries for the same date + note
                        if (trimmed.includes(noteEntry))
                            return match;
...
// =============================================================================
server.tool("getTeamsMeetingSyncState", "Get the last Teams meeting sync timestamp (watermark). Returns ISO timestamp of last successful sync.", {}, async () => {
    const stateDir = await getGlobalStateDir();
...
// =============================================================================
server.tool("appendTeamsMeetingHighlights", "Append a Teams Meeting Highlights section to today's daily note. Idempotent: updates existing section if present.", {
    date: z.string().optional().describe("Date in YYYY-MM-DD format (defaults to today)"),
    ...
    function deduplicateMeetingHighlights(existing, incoming) {
