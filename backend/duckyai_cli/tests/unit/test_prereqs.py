"""Tests for prerequisite checker (duckyai_cli.prereqs).
...
from duckyai_cli.prereqs import (
    CheckResult,
    CheckStatus,
    PrereqReport,
    check_python,
...
class TestCheckNodejs:
    @patch('duckyai_cli.prereqs._run_cmd', return_value="v22.7.0")
    ...
    @patch('duckyai_cli.prereqs._run_cmd', return_value="v16.20.0")
...
class TestCheckGit:
    @patch('duckyai_cli.prereqs._run_cmd', return_value="git version 2.44.0.windows.1")
    def test_git_found(self, mock):
        result = check_git()
        assert result.ok
        assert result.version == "2.44.0"

    @patch('duckyai_cli.prereqs._run_cmd', return_value=None)
...
class TestCheckDocker:
    ...
    @patch('duckyai_cli.prereqs._run_cmd')
...
class TestCheckAzCli:
    ...
    @patch('duckyai_cli.prereqs._run_cmd', return_value="2.73.0")
...
class TestCheckAzDevopsExt:
    @patch('shutil.which', return_value="/usr/bin/az")
    @patch('duckyai_cli.prereqs._run_cmd', return_value="1.0.1")
...
class TestCheckWorkiq:
    @patch('duckyai_cli.prereqs._run_cmd', return_value="├── @microsoft/workiq@1.2.3")
    def test_installed(self, mock):
        result = check_workiq()
        assert result.ok

    @patch('duckyai_cli.prereqs._run_cmd', return_value="(empty)")
...
class TestCheckAll:
    @patch('duckyai_cli.prereqs.check_python', return_value=CheckResult("Python", CheckStatus.OK, version="3.14"))
    @patch('duckyai_cli.prereqs.check_nodejs', return_value=CheckResult("Node.js", CheckStatus.OK, version="22.7"))
    @patch('duckyai_cli.prereqs.check_git', return_value=CheckResult("git", CheckStatus.OK, version="2.44"))
    @patch('duckyai_cli.prereqs.check_copilot_sdk', return_value=CheckResult("SDK", CheckStatus.OK))
    @patch('duckyai_cli.prereqs.check_copilot_cli', return_value=CheckResult("CLI", CheckStatus.OK))
    @patch('duckyai_cli.prereqs.check_docker', return_value=CheckResult("Docker", CheckStatus.OK))
    @patch('duckyai_cli.prereqs.check_az_cli', return_value=CheckResult("az", CheckStatus.OK))
    @patch('duckyai_cli.prereqs.check_az_devops_ext', return_value=CheckResult("ext", CheckStatus.OK))
    @patch('duckyai_cli.prereqs.check_workiq', return_value=CheckResult("wiq", CheckStatus.OK))
    ...
    def test_includes_vault_checks_when_path_given(self, tmp_path):
        with patch('duckyai_cli.prereqs._run_cmd', return_value="ok"):
...
class TestAutoFix:
    def test_fixes_az_extension(self):
        ...
        with patch('duckyai_cli.prereqs._run_cmd', return_value="installed"):
