...
from duckyai.main.orchestrator import run_orchestrator_daemon
from duckyai.main import orch_cmd
...
class TestOrchestratorFunctions:
    ...
    @patch('duckyai.main.orchestrator.Orchestrator')
    @patch('duckyai.main.orchestrator.Config')
    @patch('duckyai.main.orchestrator.signal.signal')
    ...
    @patch('duckyai.main.orchestrator.Path.cwd')
    def test_run_orchestrator_daemon_defaults_to_cwd(self, mock_cwd, temp_vault):
        ...
        with patch('duckyai.main.orchestrator.Config') as mock_config_class, \
             patch('duckyai.main.orchestrator.Orchestrator') as mock_orchestrator_class:
...
def test_cleanup_orchestrator_processes_fresh_start_terminates_matching_process(monkeypatch, tmp_path):
    ...
    proc = _FakeProc(123, vault_root, ["python", "-m", "duckyai", "-o"])
...
def test_cleanup_orchestrator_processes_keeps_healthy_process_when_not_fresh(monkeypatch, tmp_path):
    ...
    healthy_proc = _FakeProc(123, vault_root, ["python", "-m", "duckyai", "-o"])
    stale_proc = _FakeProc(456, vault_root, ["python", "-m", "duckyai", "-o"])
...
@patch('duckyai.main.orch_cmd.get_duckyai_launch_cmd', return_value=['duckyai', '-o'])
@patch('duckyai.main.orch_cmd.subprocess.Popen')
