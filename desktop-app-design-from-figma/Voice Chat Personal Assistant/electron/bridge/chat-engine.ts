...
/** Callback to push notifications to the renderer (toast). */
export type NotifyFn = (msg: { type: "success" | "error" | "info"; title: string; body?: string }) => void;
...
export class ChatEngine {
  ...
  private buildTools() {
    ...
    const mcpTool = <T>(
      ...
      mapArgs?: (args: T) => Record<string, unknown>,
    ) =>
      defineTool(name, {
        description,
        parameters,
        ...
        handler: async (args) => {
          ...
          orch.triggerAgent(args.agent, { file: args.file, lookback: args.lookback })
            .then((r) => {
              ...
              this.notify?.({
                type: "success",
                title: `${args.agent} Agent Triggered`,
                body: `${args.agent} agent has started processing.`,
              });
            })
            .catch((e) => {
              console.error(`[tool] triggerAgent ${args.agent} failed:`, e?.message);
              this.notify?.({
                type: "error",
                title: `${args.agent} Agent Failed`,
                body: e?.message ?? "Unknown error",
              });
            });
          return `Agent ${args.agent} triggered. It is now running in the background.`;
        },
      }),
    ];
  }
  ...
  async sendMessage(userText: string): Promise<string> {
    ...
    this.sendLock = new Promise<void>((r) => { resolve = r; });
  ...
  private async doSend(userText: string, isRetry = false): Promise<string> {
    let session: any;
    try {
